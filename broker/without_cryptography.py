...
while True:
    ...
    initial_msg = serverSocket.recv(1024).decode()
    ...
    if initial_msg[0] == 'CONNECT':
        ...
        if int(initial_msg[1]) in Sids:
            ...
            con.send(str(Ci_to_send).encode())

            Ri_dec = con.recv(1024).decode()
            ...
            if str(Ris[int(initial_msg[1])]) == str(Ri_dec):
                ...
                Hlogin_pass_id_dec = con.recv(1024).decode()
        ...
        serverSocket.send(str(Ci).encode())

        Ri = serverSocket.recv(1024).decode()

        Sid = randint(11111111, 99999999)
        serverSocket.send(str(Sid).encode())
